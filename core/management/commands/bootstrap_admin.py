...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not password:
            self.stdout.write(self.style.WARNING('DJANGO_SUPERUSER_PASSWORD is not set. Skipping admin bootstrap.'))
            return
        ...
        user, created = User.objects.get_or_create(
            ...
            defaults={
                ...
                'status': getattr(User.Status, 'ACTIVE', 'active'),
            },
        )
        user.email = email
        user.is_staff = True
        ...
        if hasattr(user, 'status'):
            user.status = getattr(User.Status, 'ACTIVE', 'active')
